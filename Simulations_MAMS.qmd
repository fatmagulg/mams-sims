...
plot(density(size$total_N), main = "Density of sample size", xlab = "Total (Running) Sample Size")
...
boundary_plotter <- function(l, u){
  ...
  plot(x = x, y = u, type = 'b', ylim = c(-1, ymax), xaxt = 'n',
       xlab = "Interim analysis", ylab = "Z-statistic", main = "Stopping bounds for TAILoR trial")
  axis(side = 1, at = x)
...
wj_bounds <- mams_replicates2(l = c(0, 0, 2.182), u = c(3.779, 2.672, 2.182), mu = LFC, n = c(261, 326, 396), r = 2)$ESS_trial2

# lan demetz obf boundaries:
landemetz_obf <- mams_replicates2(l = c(0, 0, 2.154), u = c(3.9849, 2.7108, 2.154), mu = c(0.15, 0.1, 0.1, 0.1), n = c(261, 326, 396), r = 2)$ESS_trial2

rbind(wj_bounds, landemetz_obf)


mams_replicates2(l = c(0, 0, 2.1434), u = c(4.5008, 2.8866, 2.1434), mu = c(0.15, 0.1, 0, 0), n = c(470, 456, 396), r = 2, sd = 0.33)

# rossini stages 5-
mams_replicates2(l = c(0, 0, 2.1543), u = c(3.9849, 2.7108, 2.1543), mu = c(0.15, 0.1, 0, 0), n = c(rep(315,3)), r = 2, sd = 0.33)
```

```{r}
HG <- c(0.15, 0.15, 0.15, 0.15)  # All treatments same as control
LFC <- c(0.15, 0.10, 0.10, 0.10)  # Effect in all treatments

n <- c(470, 456, 396)
r <- 2
u <- c(-3.8484, -2.6348, -2.1608)     # futility
l <- c(3.8484, 2.6348, -2.1608)

res <- mams_replicates2(l = l, u = u, mu = c(0,0,0,0), n = n, sd = 0.33, r = r, nrep = 1000)
res
#mams_replicates2(l = c(0, 0, 2.1434), u = c(4.5008, 2.4866, 2.1434), mu = c(0,0.05,0,0), n = c(rep(284,3)), r = 2)
...
n <- c(470, 456, 396)
r <- 2
l <- c(0, 0, 02.1608)     # futility
u <- c(3.8484, 2.6348, 2.1608)

mams_replicates2(l = l, u = u, mu = c(0,0.05,0.01,0.01), n = n, sd = 0.33, r = r, nrep = 1000)
```
