...
trial <- mams_simulator(mu = mu, sd = 0.3, n = n, silent = FALSE)
...
#--------- loop over stages 
  for(j in 1:J){
    y[[control_id]] <- c(y[[control_id]], rnorm(n=n[j], mean = mu[control_id], sd = sd))
...
```{r}
mams_replicates2 <- function(l, u, mu, sd = 1, n, nrep = 1000){
  resultsmat <- matrix(data = NA, nrow = nrep, ncol = 2)
  
  for(i in 1:nrep){
    rep <- mams_ef(l = l, u = u, sd = sd, n = n, mu = mu)
  ...
  return(list(early_stop_prob = p_earlystop, ESS_per_arm = ESS_arm, ESS_trial = ESS_trial))
}
```
...
HG <- c(rep(0,5))
mams_replicates2(l = c(0, 0, 2.182), u = c(3.779, 2.672, 2.182), n = rep(30,3), mu = LFC )$early_stop_prob
```
...
results <- rbind(twostage, threestage, fourstage)
...
plot(1:ncol(results), results[1,], type = "o", col = colours[1], pch = 16,
     ...
     main = "ESS Across Different Scenarios")
for (i in 2:nrow(results)) {
  lines(1:ncol(results), results[i,], type = "o", col = colours[i], pch = 16)
}
axis(side = 1, at = 1:ncol(results), labels = colnames(results))

legend("topright", legend = c("Two-stage", "Three-stage", "Four-stage"), title = "Scenario", col = colours, pch = 16)
```
