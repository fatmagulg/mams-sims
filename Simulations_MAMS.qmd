...
```{r}
n <- c(201, 226, 517)
ldBounds(t = cumsum(n)/sum(n), alpha = 0.05/7 , sides = 1, iuse = 1)
```
...
for(i in 1:length(effects)){
  mu = c(0, 0.05, rep(effects[i],6)) # treatment 1 ihas target diff, others have delta_0
...
plot(effects, power_est, type = 'l', main = "LFC Power for delta1 = 0.05 against delta0 candidates",  xlab = "delta0 candidates")
abline(h = 0.5, lty = 2, col = 'gray')
abline(v = 0.03, lty = 2, col = 'gray')
```
...
HG <- c(rep(0,8))
multiple <- c(0, 0.05, 0.05, 0.05, 0.01, 0.01, 0.01, 0.01)
