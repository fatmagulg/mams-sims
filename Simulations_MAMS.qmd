...
How $\alpha$-spending functions control the overall Type-I error rate

```{r, warning=FALSE}
n = c(1, 31,24,46,13) # first sample is 1 so that plot starts at (0,0)
...
plot(obf_bounds$time, obf_bounds$exit.pr, type = 'o', lty = 3, pch = 16, ylim = c(0, bounds$alpha), 
...
legend('bottomright', legend = c("Overall alpha","Pocock", "O'Brien-Fleming"), lty = c(2,1,3), pch = c(NA, 16, 16), col = c('darkblue', 'black', 'black'), bty = 'n')
```
...
```{r}
boundary_plotter <- function(l, u, ymax = NULL, ymin = NULL, main = NULL){
  x = seq(1, length(u))
  ...
  plot(x = x, y = u, type = 'b', ylim = c(ymin, ymax), xaxt = 'n',
       xlab = "Interim analysis", ylab = "Z-statistic", main = main)
  axis(side = 1, at = x)
...
boundary_plotter(l = wj_l, u = wj_u)
```
...
boundary_plotter(l = fourstage$bounds[[1]], u = fourstage$bounds[[2]])
boundary_plotter(l = my_l, u = my_u)
```
...
plot(effects, power_est, type = 'l', main = "Power against delta0 candidates",  xlab = "delta0 candidates", ylab = "Power")
abline(h = 0.5, lty = 2, col = 'gray')
abline(v = 0.0195, lty = 2, col = 'gray')
```

```{r}
n = c(201, 226, 517)
r <- 2
mu = matrix(data = c(0, 0.05, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02), ncol = 1)
...
```{r}
boundary_plotter(pocock$bounds[[1]], pocock$bounds[[2]])

boundary_plotter(obf_1$bounds[[1]], obf_1$bounds[[2]])

boundary_plotter(obf_2$bounds[[1]], obf_2$bounds[[2]])
```
...
barplot(PES_p,
        ...
        main = "Stopping probability by scenario",
        ylab = "Probability")
barplot(PES_1,
        beside = TRUE,
        #main = "",
